...
3) The FU signal has multiple functions and, by default, is configured as output. To put the sensor into programming mode, the unload + tangle sensor buttons must be pressed for about 4 seconds. The sensor ring will start flashing with a white color, indicating the programming mode is activated.
